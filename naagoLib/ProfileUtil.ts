...
import { createCanvas, loadImage } from "npm:canvas@^3.0.0";
...
class Profile {
  ...
  async getTheme(): Promise<Theme> {
    ...
    const themeFile = readFileSync(`./themes/${themeName}.json`, "utf-8");
  ...
  async getSocialMedia(isMe: boolean): Promise<EmbedBuilder> {
    const client = this.interaction.client as Client;
